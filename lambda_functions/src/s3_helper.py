...
def list_folders(bucket_name, prefix, client=None):
...
def key_exists(bucket, key, s3=None):
...
def get_all_files(bucket, prefix, folder='', s3_client=None):
    """Get all files in a given bucket (and potentially a given sub-folder)

    Args:
        bucket (string): S3 bucket to look in
        prefix (string): sub-folder
        folder (str, optional): Additional prefix (should probably be refactored?). Defaults to ''.

    Returns:
        list[dict]: The list of files contained in the bucket (/prefix if specified)
    """
...
def read_csv_boat_status(bucket, key, s3=None):
...
def csv_to_string(bucket, key, s3=None):
    if s3 is None:
        logger.debug('Creating S3 resource')
        s3 = boto3.resource('s3')
    logger.debug('Getting object')
    obj = s3.Object(bucket, key)
    logger.debug('Getting string')
    csv_str = obj.get()['Body'].read().decode('utf-8')
    return csv_str


def read_concat_csv_boat_status(bucket, key, s3=None):
    """Get fleet data (same as boat status but contain several lines).
    TODO: merge with read_csv_boat_status?
    ...
    Returns:
        ...
        lastCalcDate  lat lon heading speed   twa tws twd sail    distanceFromStart   distanceToEnd   credits timeToNextCards aground
    """
    csv_str = csv_to_string(bucket, key, s3)
    reader = csv.reader(csv_str.split('\n'), delimiter=',')
    data = []
    for row in reader:
        if row != []:
...
def get_waypoints(bucket, object_key):
    """Get the waypoints of a given isochrones result file

    Args:
        bucket (string): S3 bucket to look in
        object_key (string): The isochrones result S3 key

    Returns:
        list[[folat, float]]: The waypoints as a list [[x, y], ...]
    """
    s3 = boto3.resource('s3')
